...
class DatasetConfig:
    name: str
    text_col: str
    raw_label_col: Optional[str]  # may be None for multi-label datasets
...
def map_to_severity(dataset: str, row: pd.Series, *, raw_label_col: Optional[str]) -> str:
    """
    Map dataset-specific labels into our unified severity space: 'high' | 'medium' | 'none'.
    """
    if dataset == "davidson":
        cls_id = int(row[raw_label_col])  # type: ignore[arg-type]
        if cls_id == 0:
            return "high"
        if cls_id == 1:
            return "medium"
    ...
    if dataset == "cyberbullying_tweets":
        lbl = str(row[raw_label_col]).strip()  # type: ignore[arg-type]
    ...
    if dataset == "jigsaw":
        ...
        # Treat "severe_toxic / threat / identity_hate" as high,
        # "toxic / insult / obscene" as medium (if not already high).
        flags = {
...
def main():
    ...
    # Drop exact duplicates by text to reduce leakage
